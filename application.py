...
@app.route('/home.json/movie_by_category')
def movieByCategoryJSON():
    categories = session.query(Movies).order_by(Movies.category_id.asc())
    return jsonify(movies_by_category=[c.serialize for c in categories])
